...
public class Toy {
  ...
  public static void complete( CollaborativeFilteringSystem cfs, TIntFloatHashMap u ) {
    float[] completed = cfs.completeUser( new TIntFloatHashMap(u));
  ...
  public static void main( String[] args ) {
    ...
    if ( deterministic ) {
      ...
      if ( secondorder )
        // second-order system
        for ( int k = 0; k < N; ++k ) {
          TIntFloatHashMap copy = new TIntFloatHashMap(completeUser);
          copy.remove( k + 1 );
          uset.put( k + 1, copy );
        }

      for ( int k = 0; k < N; ++k ) {
        TIntFloatHashMap copy = new TIntFloatHashMap(completeUser2);
